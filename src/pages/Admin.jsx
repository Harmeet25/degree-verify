...
export default function Admin({ wallet, contract, certificates, setCertificates }) {
  ...
  async function handleBatchIssue() {
    ...
    try {
      ...
      setSuccess(`✓ ${newCerts.length} certificate${newCerts.length !== 1 ? "s" : ""} issued and recorded on blockchain!`);
            ...
            {loading && (
              <div style={{ marginBottom:"1rem" }}>
                ...
                <div style={{ height:6, background:"var(--ivory3)" }}>
                  <div style={{ height:"100%", width:`${progress}%`, background:"linear-gradient(90deg,var(--navy),var(--gold))", transition:"width 0.3s ease" }} />
                </div>
              </div>
            )}
        ...
        {certificates.length > 0 && (
          <div style={{ marginTop:"3rem" }}>
            <div style={{ fontFamily:"var(--f-label)", fontSize:"0.65rem", letterSpacing:"3px", textTransform:"uppercase", color:"var(--ink-light)", fontWeight:700, marginBottom:"0.75rem" }}>
              All Issued Certificates — {certificates.length} record{certificates.length!==1?"s":""}
              <span style={{ marginLeft:"0.5rem", fontSize:"0.6rem", color:"var(--sage)" }}>● Saved in browser</span>
            </div>
