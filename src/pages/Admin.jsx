...
function generateCertId(name, enrol, year) {
  const initials = name.split(" ").map(w => w[0]).join("").toUpperCase().slice(0, 3);
  return `CERT-${initials}-${year}-${enrol.slice(-4).toUpperCase()}`;
}
...
export default function Admin({ wallet, contract, certificates, setCertificates }) {
  ...
  async function handleBatchIssue() {
    ...
    try {
      // Enrich each row with hash + certId
      const enriched = rows.map(r => ({
        ...r,
        university,
        certHash : generateCertHash(r),
        certId   : generateCertId(r.studentName, r.enrolmentNumber, r.year),
      }));

      if (contract) {
        // ── On-chain batch ──────────────────────────────────────────────────
        const students      = enriched.map(r => ethers.isAddress(r.studentWallet) ? r.studentWallet : ZERO_ADDR);
        const enrolments    = enriched.map(r => r.enrolmentNumber);
        const names         = enriched.map(r => r.studentName);
        const degrees       = enriched.map(r => r.degree);
        const fields        = enriched.map(r => r.fieldOfStudy);
        const universities  = enriched.map(r => r.university);
        const years         = enriched.map(r => parseInt(r.year) || new Date().getFullYear());
        const certHashes    = enriched.map(r => r.certHash);
        const certIds       = enriched.map(r => r.certId);

        setProgress(20);
        const tx = await contract.batchIssueCertificates(
          students, enrolments, names, degrees, fields,
          universities, years, certHashes, certIds
        );
        setProgress(60);
        await tx.wait();
        setProgress(100);
          ...
          <div className="form-sheet-body">
            ...
            <div className="field-group">
              ...
              >
                ...
                <div style={{ fontFamily:"var(--f-label)", fontSize:"0.85rem", color:"var(--ink-mid)", letterSpacing:"0.5px" }}>
                  Click or drag &amp; drop your Excel file here
                </div>
                <div style={{ fontSize:"0.72rem", color:"var(--ink-light)", marginTop:"0.4rem" }}>
                  Required columns: <strong>Student Name</strong>, <strong>Enrolment Number</strong>, <strong>Course</strong>, <strong>Year</strong> &nbsp;·&nbsp; Optional: Grade, Wallet Address
                </div>
            ...
            {loading && (
              <div style={{ marginBottom:"1rem" }}>
                ...
                <div style={{ height:6, background:"var(--ivory3)", borderRadius:0 }}>
                  <div style={{
        ...
        {preview && rows.length > 0 && (
          <div style={{ marginTop:"2rem" }}>
            <div style={{
              ...
              <button className="btn-secondary" style={{ padding:"0.4rem 1rem", fontSize:"0.7rem" }} onClick={() => { setRows([]); setPreview(false); }}>
                ✕ Clear
              </button>
            </div>
            ...
            <div className="ledger-table-wrap" style={{ maxHeight:420, overflowY:"auto" }}>
              <div style={{
                display:"grid",
                gridTemplateColumns:"40px 1fr 1.2fr 1fr 80px 120px",
                background:"var(--navy)", padding:"0.7rem 1.2rem", gap:"1rem"
              }}>
                {["#","Enrolment","Student Name","Course","Year","Grade"].map(h => (
          ...
          <div style={{ marginTop:"3rem" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"1rem", marginBottom:"1.5rem" }}>
              {[
                ...
                <div key={label} style={{
                  ...
                  padding:"1.1rem 1.4rem", boxShadow:"0 1px 6px var(--shadow)"
                }}>
            ...
            <div className="ledger-table-wrap">
              <div className="ledger-header-row">
                ...
                <div className="ledger-header-cell">Enrolment</div>
                <div className="ledger-header-cell">Student</div>
                <div className="ledger-header-cell">Degree</div>
